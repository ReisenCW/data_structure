...
FamilyTree& FamilyTree::operator=(const FamilyTree& f) {//重载=运算符(深拷贝)
...
FamilyTree::FamilyTree(const FamilyTree& f) :name(f.name), father(father), maxChildCount(f.maxChildCount), childCount(f.childCount), children(nullptr) {
	//深拷贝
	if (f.children != nullptr) {
		children = new FamilyTree[childCount];
		for (int i = 0; i < childCount; i++) {
...
void FamilyTree::ResizeChild(int newSize) {
...
void FamilyTree::DeleteChild(const String& name) {
	for (int i = 0; i < childCount; i++) {
...
public:
	...
	FamilyTree* FindMember(const String& targetName, FamilyTree* Node = nullptr);
	void BuildFamily();
...
void FamilyManager::BuildFamily() {
	cout << "请输入要建立家庭的人的姓名: ";
	String fatherName;
	cin >> fatherName;
...
void FamilyManager::AddFamilyMember() {
...
void FamilyManager::DeleteFamilyMember() {
...
void FamilyManager::SearchFamilyMember() {
...
void FamilyManager::ModifyMemberName() {
...
void FamilyManager::DismissPartialFamily() {
	cout << "请输入要解散家庭的人的姓名: ";
	String fatherName;
	cin >> fatherName;
	FamilyTree* father = FindMember(fatherName);
